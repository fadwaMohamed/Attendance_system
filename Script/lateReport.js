...
import { reportsEmp, allEmpReports } from './reports.js'
            ...
            if(reportIndex != -1) {
                ...
                for(let j=0; j<currentEmpReport.length; j++)
                {
                    ...
                    if(currentEmpReport[j].date[0] == year && currentEmpReport[j].date[1] == month) {
                        if(currentEmpReport[j]['late'][2] < 0) noTimes++;
                        flag = 1;
                    }
                }
                if(flag) {
                    document.getElementById("TableL_E").style.display = 'table';
                    addRowReport(tBody, employees[i], noTimes==0 ? '--': noTimes);
...
function addRowReport(tBody, currentEmp, noTimes) 
{
